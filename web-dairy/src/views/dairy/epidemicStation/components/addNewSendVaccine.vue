...
<template>
  <div class="addNewSendVaccine">
    <Button type="primary" size="small" @click="$jump('/arrangeVaccine')" style="float:right;">返回</Button>
    <div style="width:70%;">
      ...
      <Form ref="addSendVaccineValidate" :model="addSendVaccineValidate" :rules="ruleAddSendVaccineValidate" :label-width="170">
        <FormItem label="领取防疫人员" prop="recivePerson">
          <Select v-model="addSendVaccineValidate.recivePerson" clearable style="width:35%;" placeholder="请选择领取防疫人员">
            <Option v-for="item in epidemicList" :value="item.label" :key="item.label">{{ item.label }}</Option>
          </Select>
        </FormItem>
        <FormItem label="员工所属区域" prop="blongArea">
          <Select v-model="addSendVaccineValidate.blongArea" clearable placeholder="请选择区域" style="width:50%;">
